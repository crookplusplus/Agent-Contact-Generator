...
const updateAgentContactStatus = async (req, res) => {
  ...
  try {
    ...
    res.json({
      ...
      upAgent: agent._id,
      change: change,
      mssg: "Agent contact status updated",
    });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: "Server error" });
...
//function to update agent information in database
//agent id is pased in the url
const updateAgentInfo = async (req, res) => {
  ...
  try {
    ...
    res.json({ message: "Agent updated", upList: agent.APICall_id, upAgent: agent._id });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: "Server error" });
  }
            ...
            table: {
              ...
              body: [
                ...
                ...agentList.map((agent) => [
                  ...
                  agent.nick_name,
                  agent.broker,
                  `${agent.mobile ? `M:${agent.mobile}` : ""} ${
                    agent.business ? `B:${agent.business}` : ""
                  }`,
                  agent.email,
                  agent.address,
